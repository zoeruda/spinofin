...
- **Container-baked tooling, wordlists & databases**: declared in [`custom/distrobox/spinofin-kali.ini`](custom/distrobox/spinofin-kali.ini) and managed by recipes in [`custom/ujust/kali-container.just`](custom/ujust/kali-container.just). Baked into the container at assemble time:
  ...
  - `ujust toggle-nmap` — interactively pick where `nmap` is installed: **container-only** (default — full raw-socket scans via `CAP_NET_RAW`, run with the `kali` alias), **host-only** (unprivileged `-sT` connect scans via brew, pairs with `rustscan`'s auto-nmap pipe), **both**, or **none**. Manages the *live* state on each side (`brew install/uninstall`, container `apt install/remove`) — it does not edit `spinofin-kali.ini`, so `ujust rebuild-kali` always re-creates the container with `nmap` baked in per the manifest regardless of your last choice; re-run `toggle-nmap` after a rebuild if you want something other than container-only.

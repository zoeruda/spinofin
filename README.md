...
Heavy pentest tooling lives in a single shared, rootful Kali container. Provision it once, then pull tool families on demand — or run `ujust super-kali` to do the whole first-time setup below in one pass:

```bash
ujust setup-kali              # build the shared Kali (kali-rolling) container
ujust setup-metasploit        # initialise the Metasploit database (msfdb) inside it -- run this ONCE
ujust setup-powerview         # install powerview.py inside the container via pipx
ujust setup-kerbrute          # install kerbrute inside the container via go install
...
ujust upgrade-kali            # update the container's apt tooling in place (keeps your data)
...
- **Container-baked tooling, wordlists & databases**: declared in [`custom/distrobox/spinofin-kali.ini`](custom/distrobox/spinofin-kali.ini) and managed by recipes in [`custom/ujust/kali-container.just`](custom/ujust/kali-container.just). Baked into the container at assemble time:
  ...
  - `ujust super-kali` — **the one-command path**: runs `setup-kali`, `setup-metasploit`, `setup-bloodhound`, `setup-powerview`, `setup-kerbrute`, `export-impacket`, and `link-wordlists` in that order (the same order `setup-kali`'s own "next steps" hints at) via `ujust <recipe>` calls, printing a `[n/7]` progress banner for each. Every step it calls is independently idempotent, so re-running `super-kali` is always safe. This is a heavy, long-running first run — kali-rolling, Metasploit, BloodHound + neo4j, and 1 GB+ of wordlists.
  - `ujust setup-metasploit` — initialise the Metasploit `msfdb` database. You only need this **once**: it enables `postgresql.service` inside the container (Kali ships it disabled by default), so PostgreSQL comes back on its own after every later reboot — just run `msfconsole` directly. `msfdb init`, the one command that touches the database schema, is never called again after this first run, so there's no reboot-time path that could reset it.
  - `ujust setup-powerview` — install [powerview.py](https://github.com/aniqfakhrul/powerview.py) inside the container via a container-local `pipx` install isolated to `/opt/spinofin/pipx` (NOT `~/.local/bin`, which is shared with the host's own brew-managed pipx — see the recipe comments for why). Requires `libkrb5-dev` + `python3-dev` headers baked into the container; `gssapi` ships no Linux manylinux wheels so host-side pipx cannot install this. The install is **pinned to a fixed upstream commit** (supply-chain hardening — reproducible re-installs, and no silently pulling a fresh HEAD on a later re-run/rebuild), and is intentionally **excluded from `ujust upgrade-kali`**, so once installed it stays frozen at that commit until you bump the SHA in the recipe and re-run it. Idempotent — a stamp file tracks the installed commit, so a re-run skips the actual `pipx install` (a full git clone plus a from-source `gssapi` build) once already at the pinned commit, only reinstalling if that stamp is missing/stale or the pin itself changes. The installed console script is `powerview` (the repo is named `powerview.py`, the command is not). Use `kali powerview --help` or enter the container to run it.
  - `ujust setup-kerbrute` — install [kerbrute](https://github.com/ropnop/kerbrute) inside the container via `go install github.com/ropnop/kerbrute@<pinned-commit>`, the same shape as `setup-powerview`: `GOPATH`/`GOBIN` are isolated to `/opt/spinofin/go` (NOT `$HOME/go`, keeping the container's Go module cache off the host-visible home directory). Requires `golang-go` baked into the container (verified/installed on demand in the recipe too, in case the container has drifted); kerbrute has no Kali apt package and stays off the host on purpose, to avoid a second host-side package manager for one tool. Like `setup-powerview`, the install is **pinned to a fixed upstream commit** (supply-chain hardening — not `@latest`) and is intentionally **excluded from `ujust upgrade-kali`**, so it stays frozen at that commit until you bump the SHA in the recipe and re-run it. Idempotent — same stamp-file skip as `setup-powerview`, so a re-run at the same pin does not repeat the `go install`. Use `kali kerbrute --help` or enter the container to run it.
  - `ujust setup-bloodhound` → `ujust bloodhound-start` — install BloodHound CE on demand, provision its PostgreSQL + neo4j databases, then start it. You only need `setup-bloodhound` **once**; after every later reboot, `bloodhound-start` alone re-runs that same proven bring-up sequence (cheap, no apt, fully offline) and starts the service — no need to repeat `setup-bloodhound`. This is enforced, not just documented: a stamp file at `/etc/bhapi/.spinofin-provisioned` inside the container (same pattern as `setup-powerview`/`setup-kerbrute`'s pinned-commit stamps) makes a re-run a no-op that points you at `bloodhound-start` instead of repeating the apt install and the full bring-up — and, importantly, instead of repeating the "log in with the default neo4j/neo4j password" instructions once you've already changed it. `ujust rebuild-kali` wipes that stamp along with the rest of the container's root filesystem, so it correctly forces a fresh `setup-bloodhound` next time. BloodHound is managed by a systemd unit, so the container runs as an **init container** (`init=true` in the `.ini`); if a command errors with *"System has not been booted with systemd as init system"*, the container predates that and needs `ujust rebuild-kali`. **Headless handling:** Kali's helpers try to pop a local browser (`xdg-open`), which has no display in the container — so both recipes neutralize that step (provisioning still completes; you open **`http://localhost:7474`** in your *host* browser the first time to set the neo4j password and update `/etc/bhapi/bhapi.json`), and `bloodhound-start` starts the stack **non-blocking** instead of running Kali's wait-for-API-then-`xdg-open` wrapper (which hangs headless). Open the UI at **`http://localhost:8080`** in your host browser. **Note on neo4j:** Kali's bloodhound package manages neo4j directly via its own binary, not as a `neo4j.service` systemd unit — so a warning mentioning `neo4j.service` (e.g. *"Unit ... not found"*) is expected and safe to ignore; the bring-up step already starts/verifies neo4j the correct (non-systemd) way.
  ...
  - `ujust link-wordlists` — stage the Kali wordlists for **host-native** tools (john, hashcat, ffuf). distrobox shares only `$HOME` (not the container's `/usr`), *and* `/usr/share/wordlists` is mostly symlinks into other packages (many dangling), so a bare symlink/copy would be broken on the host. Instead this stages **real files** under `~/.local/share/spinofin/wordlists` (linked as `~/wordlists`): it relocates the `seclists` tree there (symlinking `/usr/share/seclists` back so the container keeps its path) and decompresses `rockyou` to `~/wordlists/rockyou.txt`. Every link in that chain is re-verified and self-healed on each run, not just checked for existence — deleting `~/wordlists` (or the `seclists` copy, or a dangling `/usr/share/seclists` symlink left over from either) is recovered on the next run, reinstalling the `seclists` apt package first if needed to get a fresh copy. Use e.g. `hashcat -m 0 -a 0 hashes.txt ~/wordlists/rockyou.txt` or `john --wordlist=~/wordlists/rockyou.txt hashes.txt`, plus the real lists under `~/wordlists/seclists/Passwords/`.
  - `ujust upgrade-kali` — the **non-destructive** way to keep the container's tooling current on `kali-rolling`. Runs `apt-get update && apt-get full-upgrade` (plus `autoremove`/`clean`) *inside the existing container*, so unlike `ujust rebuild-kali` (which is `--replace` and recreates the box from scratch) it does **not** wipe anything not baked into the manifest: your `msfdb` database, the BloodHound/neo4j databases, the container-local powerview/kerbrute installs, and any `kali-tools-*` families you pulled all survive — `apt` upgrades packages but never clears the PostgreSQL/neo4j data directories. Covers every apt-managed tool in one pass (the manifest's `additional_packages` **and** any installed `kali-tools-*` metapackages). **powerview and kerbrute are deliberately NOT upgraded here** — they aren't apt packages (pipx / `go install`), and they're pinned to a fixed commit on purpose (see their setup recipes above); to move either, bump its pinned SHA and re-run that recipe. Reach for `rebuild-kali` only when you actually want a clean-slate container.
